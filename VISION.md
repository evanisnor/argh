...
### Goals
- Give engineers a **real-time pull request command center** in the terminal.
...
Displays all non-closed pull requests authored by the current user.
...
Displays pull requests where the current user is assigned as a reviewer, or is mentioned in an open review request.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  argh v0.1.0  @evanisnor  ●  github.com/evanisnor/argh          [?] help │
├──────────────────────────────────────────────────────────────────────────┤
│  MY PULL REQUESTS                                               [3 open]  │
├────┬───┬────┬──────────────────────────────┬──────────┬────┬──────┬──────┤
│ id │ 👁 │ #  │ Title                        │ Status   │ CI │ Rev  │ Age  │
├────┼───┼────┼──────────────────────────────┼──────────┼────┼──────┼──────┤
│  a │ 👁 │ 42 │ feat: add oauth flow         │ approved │ ✓  │@alice│ 2h   │  ← focused
│  b │   │ 38 │ fix: null pointer on startup │ open     │ ✗  │ @bob │ 1d   │
│  c │   │ 31 │ [draft] wip: rewrite parser  │ draft    │ ⟳  │  —   │ 4d   │
├──────────────────────────────────────────────────────────────────────────┤
│  REVIEW QUEUE                                                 [2 waiting] │
├────┬───┬────┬──────────────────────────────┬──────────┬────┬──────┬──────┤
│ id │ 👁 │ #  │ Title                        │ Author   │ CI │ Urg  │ Age  │
├────┼───┼────┼──────────────────────────────┼──────────┼────┼──────┼──────┤
│  d │   │ 55 │ chore: bump dependencies     │ @carol   │ ✓  │ ●●●  │ 3h   │
│  e │   │ 51 │ feat: dark mode toggle       │ @dave    │ ⟳  │ ●●○  │ 6h   │
├──────────────────────────────────────────────────────────────────────────┤
│  AUTOMATION QUEUE                                              [3 active] │
├──────┬────┬──────────────────────┬───────────────────┬────────────────────┤
│  id  │ #  │ Trigger              │ Action            │ Status             │
├──────┼────┼──────────────────────┼───────────────────┼────────────────────┤
│  1   │ 42 │ on:approved+ci       │ merge             │ ⟳ waiting          │
│  2   │ 31 │ on:ci-pass           │ ready-for-review  │ ⟳ waiting          │
│  3   │ 38 │ on:24h-stale         │ comment + notify  │ ⟳ scheduled        │
├──────────────────────────────────────────────────────────────────────────┤
│  > :                                           [tab: complete] [↑: hist] │
└──────────────────────────────────────────────────────────────────────────┘
```
...
Uses `terminal-notifier` on macOS, `notify-send` on Linux. Configurable per-event in `~/.config/argh/config.yaml`.
...
│                                                              │
│   ┌──────────┐    ┌───────────────┐    ┌────────────────┐   │
│   │  GitHub  │◄───│  API Client   │───►│   State Store  │   │
│   │ GraphQL  │    │  (gh token)   │    │  (in-memory)   │   │
│   └──────────┘    └───────────────┘    └───────┬────────┘   │
│                          ▲                     │            │
│                   ┌──────┘                     ▼            │
│                   │                   ┌────────────────┐    │
...
│   ├── notify/        # OS notification dispatch
...
argh auto-detects the repository from `git remote origin` in the current working directory. Use `--repo owner/name` to override.
...
- **GraphQL** for bulk PR list queries (minimize round trips, get exactly the fields needed).
...
- GraphQL query batches: My PRs + Review Queue + Check Runs in a single request.
